using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace OperatorFunctions
{
    class Program
    {
        // An operator is overloaded by declaring an operator function:

        public struct Note
        {
            int value;
            public int SemitonesFromA { get { return value; } }

            public Note(int semitonesFromA) { value = semitonesFromA; }

            public static Note operator +(Note x, int semitones)
            {
                return new Note(x.value + semitones);
            }

            // See the last example in "Equality Comparison", Chapter 6 for an example of overloading the == operator
        }

        static void Main()
        {
            Note B = new Note(2);
            Note CSharp = B + 2;
            Console.WriteLine(CSharp.SemitonesFromA);

            //Overloading an assignment operator automatically supports the
            //corresponding compound assignment operator. In 
            //our example, since we overrode +, we can use+= too:
            CSharp += 2;
            Console.WriteLine(CSharp.SemitonesFromA);
        }
    }
}
